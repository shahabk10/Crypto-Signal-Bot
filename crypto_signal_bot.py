...
# ── CONFIG ─────────────────────────────────────────────────────────────────────
LCW_API_KEY = "8be32703-67a3-462b-b161-dd8d6e3e17df"
...
# ── COIN META ──────────────────────────────────────────────────────────────────
# LCW uses simple ticker codes: BTC, ETH, DOGE, PEPE etc.
META = {
    ...
    "BRETT":  ("Brett",      "BRETT/USDT",  "Meme",     "B",  "#3B82F6"),
}
...
def lcw_history(code: str, timeframe: str = "1D") -> pd.DataFrame:
    ...
    # Ensure meaningful candle body — LCW sometimes gives identical O/H/L/C
    for i in out.index:
        o, h, l, c = out.at[i,"Open"], out.at[i,"High"], out.at[i,"Low"], out.at[i,"Close"]
        if h == l:
            spread = o * 0.002
            out.at[i,"High"]  = o + spread
            out.at[i,"Low"]   = o - spread
            out.at[i,"Open"]  = o + spread * 0.3
            out.at[i,"Close"] = o - spread * 0.3
...
def rsi_series(closes: np.ndarray) -> list:
    out = []
...
def analyze(raw: str, timeframe: str = "1D") -> dict:
    ...
    m = META.get(code, (name_api, code+"/USDT", "Crypto", "●", "#848E9C"))
    return {
        "code": code, "name": m[0], "pair": m[1], "cat": m[2],
        "icon": m[3], "color": m[4],
        "live": live, "cp": cp, "pct": pct24,
        "vol24": vol24, "cap": cap,
...
def build_chart(d: dict):
    ...
    # ── Fix degenerate candles — ensure visible body ──
    for idx in df.index:
        o = df.at[idx, "Open"]
        c = df.at[idx, "Close"]
        h = df.at[idx, "High"]
        l = df.at[idx, "Low"]
        # If body too small relative to price, force minimum 0.1% body
        if abs(c - o) < (h - l) * 0.05 or h == l:
            mid = (o + c) / 2
            spread = max((h - l) * 0.3, mid * 0.001)
            df.at[idx, "Open"]  = mid + spread / 2
            df.at[idx, "Close"] = mid - spread / 2
            df.at[idx, "High"]  = max(h, mid + spread)
            df.at[idx, "Low"]   = min(l, mid - spread)
...
H("""<div style="background:#161A1E;border-bottom:1px solid #2B3139;
  ...
  <span style="font-size:1.15rem;font-weight:700;color:#F0B90B;margin-right:32px">
    📊 CryptoSignal
  </span>
  ...
  <div style="margin-left:auto;display:flex;align-items:center;gap:10px">
    <div style="background:#0ECB8118;border:1px solid #0ECB81;color:#0ECB81;
      font-size:.7rem;font-weight:600;padding:3px 10px;border-radius:4px;
      letter-spacing:.04em">● LIVE DATA</div>
    <div style="background:#F0B90B18;border:1px solid #F0B90B;color:#F0B90B;
      font-size:.7rem;font-weight:600;padding:3px 10px;border-radius:4px;
      letter-spacing:.04em">LCW API</div>
  </div>
</div>""")
...
H("""<div style="background:#161A1E;border-bottom:1px solid #2B3139;
  ...
  <span style="margin-left:auto;color:#2B3139;font-size:.65rem">
    Powered by LiveCoinWatch API &nbsp;|&nbsp; Not financial advice
  </span>
</div>""")
...
H(f"""<div style="padding-bottom:14px;border-bottom:1px solid #2B3139;
  ...
  <div>
    ...
    <div style="font-size:.75rem;color:#474D57;margin-top:3px">
      RSI-14 + SMA-20 &nbsp;·&nbsp; Auto TP/SL &nbsp;·&nbsp;
      3-month chart &nbsp;·&nbsp; Real-time via LiveCoinWatch
    </div>
  </div>
...
for cat, coins in PICKS.items():
    H('<div style="font-size:.67rem;color:#474D57;font-weight:600;'
    ...
    for coin in coins:
        row += (
            '<a href="?chip=' + coin + '" target="_self" style="'
            'display:inline-flex;align-items:center;justify-content:center;'
            'background:#1E2329;color:#848E9C;border:1px solid #2B3139;'
            'border-radius:4px;font-size:.75rem;font-weight:500;padding:5px 13px;'
            'text-decoration:none;white-space:nowrap;font-family:IBM Plex Mono,monospace;'
            'cursor:pointer;transition:all .15s"'
            'onmouseover="this.style.borderColor=\'#F0B90B\';'
            'this.style.color=\'#F0B90B\';this.style.background=\'rgba(240,185,11,0.06)\'"'
            'onmouseout="this.style.borderColor=\'#2B3139\';'
            'this.style.color=\'#848E9C\';this.style.background=\'#1E2329\'">'
            + coin + '</a>'
        )
    row += "</div>"
    H(row)
...
if clicked and sym_inp:
    ...
    with st.spinner(f"⏳ {sym_inp.upper().strip()} [{st.session_state.tf}] data fetch ho raha hai..."):
        try:
            ...
            st.session_state.error = "Error: " + str(ex)
            st.session_state.skey += 1
            st.rerun()

# ── ERROR ──────────────────────────────────────────────────────────────────────
if st.session_state.error:
    H('<div style="background:rgba(246,70,93,0.07);border:1px solid rgba(246,70,93,0.3);'
...
if st.session_state.result:
    ...
    # Schema guard
    if not {"code","stype","live","pct","cp","tp","sl"}.issubset(d.keys()):
    ...
    pair   = d.get("pair",  code)
    cat    = d.get("cat",   "Crypto")
    icon   = d.get("icon",  "●")
    color  = d.get("color", "#848E9C")
    live   = d.get("live",  0.0)
    cp     = d.get("cp",    0.0)
    pct    = d.get("pct",   0.0)
    tp     = d.get("tp",    0.0)
    ...
    pct_str = ("+" if pu else "") + f"{pct:.2f}%  (24h)"
    ...
    # Price header
    H('<div style="background:#161A1E;border:1px solid #2B3139;border-radius:8px;'
      'padding:18px 22px;margin-bottom:14px;'
      'display:flex;flex-wrap:wrap;gap:20px;align-items:center;justify-content:space-between">'

      '<div style="display:flex;align-items:center;gap:14px">'
      '<div style="width:42px;height:42px;border-radius:50%;'
      'background:' + color + '22;color:' + color + ';font-size:1.3rem;'
      'display:flex;align-items:center;justify-content:center">' + icon + '</div>'
      '<div>'
      '<div style="font-size:1.05rem;font-weight:700;color:#EAECEF">' + name +
      ' <span style="color:#474D57;font-size:.8rem;font-weight:400;margin-left:6px">' + cat + '</span></div>'
      '<div style="font-size:.76rem;color:#474D57;margin-top:2px">' + pair + '</div>'
      '</div></div>'
      ...
      'letter-spacing:-.02em;line-height:1">' + fmt(live) + '</div>'
      '<div style="color:' + pc + ';font-size:.86rem;font-weight:500;margin-top:4px">' + pct_str + '</div>'
      '</div>'

      '<div>'
      '<div style="font-size:.64rem;color:#474D57;text-transform:uppercase;'
      'letter-spacing:.06em;margin-bottom:8px">AI Signal</div>'
      '<div style="background:' + sig_bg[stype] + ';border:1px solid ' + sig_bdr[stype] + ';'
      'color:' + sig_clr[stype] + ';font-size:.84rem;font-weight:700;'
    ...
    # Stat cards (8 cards now including Vol + MCap from LCW)
    ...
    def scard(label, value, sub, clr="#EAECEF"):
        return (
            ...
            '<div style="font-size:.88rem;font-weight:600;color:' + clr + ';'
            'font-family:IBM Plex Mono,monospace">' + value + '</div>'
            '<div style="font-size:.64rem;color:#474D57;margin-top:3px">' + sub + '</div>'
            '</div>'
        )

    H('<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px">'
      + scard("Entry Price", fmt(cp),            "Last close (3mo data)")
      + scard("Take Profit ✦", fmt(tp),          "+" + f"{abs(tp_pct):.1f}% from entry", "#0ECB81")
      + scard("Stop Loss ✦",  fmt(sl),           sl_pct_str, "#F6465D")
      + scard("RSI (14)",     f"{rsi:.1f}",      rsi_lbl, rsi_c)
      + '</div>'
      '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px">'
      + scard("Risk / Reward", f"1 : {rr:.2f}", trend, rr_c)
      + scard("3M High / Low", fmt(high3m),      "Low " + fmt(low3m))
      + scard("24h Volume",    fmt_large(vol24),  "LiveCoinWatch")
      + scard("Market Cap",    fmt_large(cap),    "LiveCoinWatch")
      + '</div>')

    # Insight
    if stype == "buy":
        ins = ("Price " + fmt(cp) + " SMA-20 (" + fmt(sma) + ") se upar. "
        ...
        ins = ("RSI " + f"{rsi:.0f}" + " neutral (30-70). SMA: " + fmt(sma) + ". "
               "Clear direction nahi — confirmation ka wait karen.")

    H('<div style="background:#161A1E;border:1px solid #2B3139;'
...
if st.session_state.history:
    ...
    for h in st.session_state.history:
        ...
        rows += (
            ...
            '<span style="color:' + pc + '">' + ps + f"{pct_v:.2f}" + '%</span>'
